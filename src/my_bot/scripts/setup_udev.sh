...
trap 'rm -f "$TMP"' EXIT
{
  ...
  echo "#"
  echo "# WHICH IS WHICH, measured on the wire during that run rather than assumed:"
  echo "#"
  printf '#   %-9s %-11s %s\n' "${PORT[$ESP_DEV]}" "($ESP_DEV)" \
    "ESP32 -- answered \`e\` as: $ESP_HEARD"
  printf '#   %-9s %-11s %s\n' "${PORT[$LIDAR_DEV]}" "($LIDAR_DEV)" \
    "lidar -- unprompted stream: $LIDAR_HEARD"
  echo "#"
  echo "# The ttyUSB numbers above are a snapshot and mean nothing later: they have"
  echo "# already swapped once on this hardware. The port paths are the identity."
  echo "#"
  echo "# Re-verify after any replug, in ten seconds:"
  echo "#     scripts/motor_check.py --seconds 2.5   # or encoder_report.py --seconds 2"
  echo "# Both refuse a port that streams unprompted, which is exactly the failure"
  echo "# a crossed pair produces. Neither adapter has a unique serial (both are"
  echo "# 10c4:ea60 reporting \`0001\`), so these rules match on physical USB port"
  echo "# and are a promise about CABLING: move an adapter to another socket and"
  echo "# its name silently follows the socket, not the device. Label the sockets."
  echo
